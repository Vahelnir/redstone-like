...
// Overlay d'information sur le bloc visé
const blockInfoOverlay = document.createElement("div");
blockInfoOverlay.style.position = "absolute";
blockInfoOverlay.style.top = "10px";
blockInfoOverlay.style.right = "10px";
blockInfoOverlay.style.padding = "8px 14px";
blockInfoOverlay.style.background = "rgba(0,0,0,0.7)";
blockInfoOverlay.style.color = "#fff";
blockInfoOverlay.style.fontFamily = "monospace";
blockInfoOverlay.style.fontSize = "16px";
blockInfoOverlay.style.zIndex = "201";
blockInfoOverlay.style.borderRadius = "8px";
blockInfoOverlay.style.pointerEvents = "none";
blockInfoOverlay.textContent = "Bloc : aucun";
document.body.appendChild(blockInfoOverlay);
...
// Détection du bloc visé au survol
renderer.domElement.addEventListener("pointermove", (event) => {
  ...
  if (intersects.length > 0) {
    ...
    if (block) {
      ...
      const pos = block.position?.toStringKey?.() || "?";
      blockInfoOverlay.textContent = `Bloc : ${type}\nPosition : ${pos}`;
      return;
    }
  }
  blockInfoOverlay.textContent = "Bloc : aucun";
